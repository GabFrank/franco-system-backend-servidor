-- ===========================================================
-- FIX IVA marzo 2026 farmacia
-- 1. Backup tablas a public.backup_*_20260528
-- 2. Poblar factura_legal_item.iva (NULL -> resuelto, 10 -> 0 GLUCONEX)
-- 3. Recalcular parciales factura_legal aplicando descuento proporcional
-- 4. Verificacion
-- Todo en una transaccion. ROLLBACK si verify falla.
-- ===========================================================
...
-- BACKUP
-- ===========================================================
DROP TABLE IF EXISTS public.backup_fl_marzo_20260528;
DROP TABLE IF EXISTS public.backup_fli_marzo_20260528;

CREATE TABLE public.backup_fl_marzo_20260528 AS
SELECT * FROM financiero.factura_legal
WHERE fecha >= '2026-03-01 00:00:00' AND fecha < '2026-04-01 00:00:00';

CREATE TABLE public.backup_fli_marzo_20260528 AS
...
WHERE fl.fecha >= '2026-03-01 00:00:00' AND fl.fecha < '2026-04-01 00:00:00';

SELECT 'BACKUP fl_count' AS k, COUNT(*) FROM public.backup_fl_marzo_20260528
UNION ALL
SELECT 'BACKUP fli_count', COUNT(*) FROM public.backup_fli_marzo_20260528;

-- ===========================================================
-- PASO 1: Mapping desc -> iva con override
-- ===========================================================
...
UNION
SELECT UPPER(TRIM(descripcion_factura)) FROM productos.producto WHERE descripcion_factura IS NOT NULL AND TRIM(descripcion_factura)<>'' GROUP BY 1 HAVING COUNT(DISTINCT iva)>1;

DROP TABLE IF EXISTS tmp_di;
CREATE TEMP TABLE tmp_di AS
WITH u AS (
  SELECT UPPER(TRIM(descripcion)) AS dn, iva::int FROM productos.producto WHERE descripcion IS NOT NULL AND TRIM(descripcion)<>''
  UNION
  SELECT UPPER(TRIM(descripcion_factura)), iva::int FROM productos.producto WHERE descripcion_factura IS NOT NULL AND TRIM(descripcion_factura)<>''
)
...
INSERT INTO tmp_override VALUES
  ('TIRZEC 15MG',                            0),
  ('TG 15  MG NUEVA PRESENTACION',           0),
  ('RETRATUTIDA SYNEDICA VERDE',             0),
  ('TIRZEDRAL 15 MG',                        0),
  ('GHK-CU AMPOLLA',                         0),
  ('GLOW GHK-CU',                            0),
  ('TESTENAT DEPOT 250 MG 10ML GOLD',        5),
  ('OXANDROLANA 5 MG X 100 COM',             5),
  ('ADROLIC MESTEROLONA 25 MG X 20 COMP',    5),
  ('PROTIUM-T X 30 CAPSULAS',                5),
  ('MELATONIN 5MG',                         10);
...
-- ===========================================================
-- PASO 2: UPDATE factura_legal_item.iva
-- ===========================================================
...
SELECT 'PASO 2 candidatos' AS k, COUNT(*) FROM tmp_candidatos;
...
-- Verify post step 2
SELECT 'PASO 2 items NULL post' AS k,
...
-- ===========================================================
-- PASO 3: Recalcular parciales factura_legal con descuento proporcional
-- ===========================================================
...
SELECT 'VERIFY' AS k,
   ...
   COALESCE(SUM(fl.total_parcial_0),0)::bigint  AS sum_p0,
   COALESCE(SUM(fl.total_parcial_5),0)::bigint  AS sum_p5,
   COALESCE(SUM(fl.total_parcial_10),0)::bigint AS sum_p10,
   COALESCE(SUM(fl.iva_parcial_5),0)::bigint    AS sum_iva5,
   COALESCE(SUM(fl.iva_parcial_10),0)::bigint   AS sum_iva10,
   COALESCE(SUM(fl.total_final),0)::bigint      AS sum_final
FROM financiero.factura_legal fl
WHERE fl.fecha >= '2026-03-01 00:00:00' AND fl.fecha < '2026-04-01 00:00:00';

-- Si la suma total_final del backup == suma total_final post-fix, OK
SELECT 'PRE vs POST total_final' AS k,
   (SELECT SUM(total_final) FROM public.backup_fl_marzo_20260528)::bigint AS pre,
   (SELECT SUM(total_final) FROM financiero.factura_legal WHERE fecha >= '2026-03-01' AND fecha < '2026-04-01')::bigint AS post;
