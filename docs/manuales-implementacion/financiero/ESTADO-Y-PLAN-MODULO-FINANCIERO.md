...
- **AJ-2 (F5):** mismo poller para reconciliar `venta_credito_cuota` cobradas → `MovimientoCliente`/`Cliente.saldoActual`. ⚠️ **NO CONSTRUIDO — issue #203 (2026-08-12).** Confirmado en runtime: el cobro en efectivo de una cuota en el POS filial marca la cuota `COBRADO` (replica a central) pero `Cliente.saldoActual` en central **no baja**. Además la pantalla de cobro del desktop (Clientes → Cobrar selección) usa la mutation legacy **`cobrarVentaCredito`** que **no existe en central** y en filial es un **stub vacío** (`VentaCreditoGraphQL.java:112`) → cobrar deuda desde la app no persiste nada. El único cobro CPC vivo y correcto es `cobrarCuotaCreditoBanco` (banco), pero el desktop **no lo cablea**. Requisito nuevo: todo cobro efectivo/transferencia debe entrar obligatoriamente a una caja mayor / cuenta destino.
