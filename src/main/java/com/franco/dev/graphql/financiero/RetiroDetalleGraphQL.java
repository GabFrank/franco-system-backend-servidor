...
public class RetiroDetalleGraphQL implements GraphQLQueryResolver, GraphQLMutationResolver {
    ...
    public Optional<RetiroDetalle> retiroDetalle(Long id, Long sucId) {
    ...
    public RetiroDetalle saveRetiroDetalle(RetiroDetalleInput input) {
        ...
        if (input.getRetiroId() != null)
            e.setRetiro(retiroService.findById(new EmbebedPrimaryKey(input.getRetiroId(), input.getSucursalId()))
                    .orElse(null));
