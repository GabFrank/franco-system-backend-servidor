...
public interface VehiculoSucursalRepository extends HelperRepository<VehiculoSucursal, Long> {
    ...
    @Query(value = "select * from vehiculos.vehiculo_sucursal " +
            ...
            "order by creado_en desc", countQuery = "select count(*) from vehiculos.vehiculo_sucursal " +
                    "where (cast(:sucursalId as bigint) is null or sucursal_id = cast(:sucursalId as bigint)) " +
                    "and (cast(:responsableId as bigint) is null or responsable_id = cast(:responsableId as bigint))", nativeQuery = true)
    public Page<VehiculoSucursal> findAllBySucursalAndResponsable(
            @org.springframework.data.repository.query.Param("sucursalId") Long sucursalId,
            @org.springframework.data.repository.query.Param("responsableId") Long responsableId, Pageable pageable);
