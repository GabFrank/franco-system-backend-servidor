...
public interface FacturaLegalRepository
                extends HelperRepository<FacturaLegal, EmbebedPrimaryKey>, JpaSpecificationExecutor<FacturaLegal> {
        ...
        public Page<FacturaLegal> findByCreadoEnBetweenAndSucursalId(LocalDateTime inicio, LocalDateTime fin,
                        List<Long> sucId, String nombre, String ruc, Pageable pageable);

        @Query(value = "select fl from FacturaLegal fl where " +
                        "(fl.creadoEn between :inicio and :fin) " +
                        "and (fl.sucursalId in :sucId) " +
                        "order by fl.creadoEn")
        public List<FacturaLegal> findByCreadoEnBetweenAndSucursalId(LocalDateTime inicio, LocalDateTime fin,
                        Long sucId);
        ...
        @Query(value = "select new com.franco.dev.domain.financiero.dto.ResumenFacturasDto(count(fl.id), max(fl.numeroFactura), min(fl.numeroFactura), "
                        ...
                        "and (:nombre is null or fl.nombre like :nombre) " +
                        "and (:ruc is null or fl.ruc like :ruc) " +
                        "and (:sinNombre is null or (:sinNombre = true and fl.nombre = 'SIN NOMBRE') or (:sinNombre = false and fl.nombre != 'SIN NOMBRE'))")
        public ResumenFacturasDto findResumenFacturas(LocalDateTime inicio, LocalDateTime fin, List<Long> sucId,
                        String nombre, String ruc, Boolean sinNombre);
        ...
        // "CAST(sum(CASE WHEN f.total_parcial_5 IS NOT NULL THEN f.total_parcial_5 -
        // f.total_parcial_5 / 21 ELSE 0 END) AS TEXT) as ven_gra05, " +
        // "CAST(sum(CASE WHEN f.total_parcial_5 IS NOT NULL THEN f.total_parcial_5 / 21
        // ELSE 0 END) AS TEXT) as ven_iva05, " +
        ...
        // "CAST(CASE WHEN f.credito = true THEN 'CREDITO' ELSE 'CONTADO' END AS TEXT)
        // as form_pag, " +
        ...
        // "CAST(substring(CAST(to_char(f.creado_en, 'DD-MM-YYYY') AS TEXT) from 0 for
        // 11) AS TEXT) as ven_fecha, " +
        // "CAST(f.total_final AS TEXT) as ven_totfac, " +
        // "CAST('0' AS TEXT) as ven_exenta, " +
        // "CAST(sum(CASE WHEN f.total_parcial_10 IS NOT NULL THEN f.total_parcial_10 -
        // f.total_parcial_10 / 11 ELSE 0 END) AS TEXT) as ven_gravad, " +
        // "CAST(sum(CASE WHEN f.total_parcial_10 IS NOT NULL THEN f.total_parcial_10 /
        // 11 ELSE 0 END) AS TEXT) as ven_iva, " +
        ...
        // "where cast(f.creado_en as text) between cast(:inicio as text) and cast(:fin
        // as text) and s.id = :sucursalId " +
        // "group by td.punto_expedicion, s.codigo_establecimiento_factura,
        // f.numero_factura, f.credito, f.ruc, f.nombre, f.creado_en, f.total_final,
        // t.numero " +
