...
public interface MovimientoStockRepository
                extends HelperRepository<MovimientoStock, com.franco.dev.domain.EmbebedPrimaryKey> {
        ...
        @Query("select SUM(p.cantidad) from MovimientoStock p " +
                        "left outer join p.producto as pro " +
                        "where p.estado = true and pro.id = ?1 and p.sucursalId = ?2 and (p.creadoEn < ?3 or (p.creadoEn = ?3 and p.id < ?4))")
        ...
        public MovimientoStockCantidadAndIdDto stockByProductoIdAndSucursalIdAndLastId(Long proId, Long sucId,
                        Long lastId);
        ...
        @Query(value = "select * from operaciones.movimiento_stock p " +
                        ...
                        "order by p.creado_en desc limit ?3", nativeQuery = true)
        ...
        public MovimientoStock findByTipoMovimientoAndReferenciaAndSucursalIdAndProductoId(
                        TipoMovimiento tipoMovimiento, Long referencia, Long sucId, Long prodId);

        public List<MovimientoStock> findByTipoMovimientoAndReferenciaAndEstadoTrue(TipoMovimiento tipoMovimiento,
                        Long referencia);
        ...
        Page<ProductoSaldoDto> findProductosConCantidadPositiva(@Param("sucursalId") Long sucursalId,
                        @Param("productoId") Long productoId, Pageable pageable);
        ...
        Page<ProductoSaldoDto> findProductosConCantidadNegativa(@Param("sucursalId") Long sucursalId,
                        @Param("productoId") Long productoId, Pageable pageable);
