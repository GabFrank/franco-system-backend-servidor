...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'operaciones' AND table_name = 'nota_recepcion_item' AND column_name = 'es_bonificacion') THEN
        ALTER TABLE operaciones.nota_recepcion_item ADD COLUMN es_bonificacion BOOLEAN DEFAULT FALSE;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'operaciones' AND table_name = 'nota_recepcion_item' AND column_name = 'vencimiento_en_nota') THEN
        ALTER TABLE operaciones.nota_recepcion_item ADD COLUMN vencimiento_en_nota DATE;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'operaciones' AND table_name = 'nota_recepcion_item' AND column_name = 'observacion') THEN
        ALTER TABLE operaciones.nota_recepcion_item ADD COLUMN observacion TEXT;
    END IF;
END $$;
